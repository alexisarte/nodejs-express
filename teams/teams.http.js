...
const getTeamFromUser = async (req, res) => {
    ...
    let [teamErr, team] = await teamsController.getTeamOfUser(req.user.userId);
    if (teamErr) {
...
const addPokemonToTeam = async (req, res) => {
    ...
    let [errorAdd, response] = await to(teamsController.addPokemonToTeam(req.user.userId, pokemon));
    if (errorAdd) { 
...
exports.setTeamFromUser = setTeamToUser;
